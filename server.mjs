...
async function supabaseLogin(data){const auth=await supabase('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email:String(data.email).trim().toLowerCase(),password:data.password})});const uid=auth.user?.id;if(uid)users[String(data.email).trim().toLowerCase()]={id:uid,name:auth.user.user_metadata?.name||String(data.email).split('@')[0],email:String(data.email).trim().toLowerCase(),role:'candidate',password:data.password,cv_path:null,supabase_token:auth.access_token};return auth}
const jobs = [
